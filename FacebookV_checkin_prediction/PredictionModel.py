...
if __name__ == '__main__':
    ...
    print('Loading data ...')
    df_train = pd.read_csv('data/train.csv', dtype={'x':np.float32,
                                               'y':np.float32,
                                               'accuracy':np.int16,
                                               'time':np.int,
                                               'place_id':np.int},
                                               index_col = 0)
    df_test = pd.read_csv('data/test.csv', dtype={'x':np.float32,
                                              'y':np.float32,
                                              'accuracy':np.int16,
                                              'time':np.int,
                                              'place_id':np.int},
                                              index_col = 0)
    df_sub = pd.read_csv('data/sample_submission.csv', index_col = 0)
